...
!                  )
    ...
    use cam_history,     only: outfld, addfld, phys_decomp ! HC: the addfld and phys_decomp are added here temporarily for TOPO_Mapping
...
!#endif
    ...
    type(physics_ptend)   :: ptend            ! indivdual parameterization tendencies
#ifdef TOPOTEST
    type(physics_ptend)   :: ptend_TOPO, HC_deep_ptend, HC_shallow_ptend ! added for TOPOTEST_X runs    
#endif
    integer :: nstep                          ! current timestep number
...
#ifdef TOPOTEST    
    ! Add this variable to hold ptend%s / cpair, 
    real(r8) :: HCforcing(pcols,pver)
#endif

    ! define a sgh_htfrac(pcols), for topogaphy induced rainfall biases test, HC 20180619
    real(r8) sgh_htfrac(pcols)    
...
#ifdef TOPOTEST
    HC_shallow_ptend = ptend
#endif
        
    call physics_update (state, tend, ptend, ztodt)

#ifdef TOPOTEST
    ptend_TOPO = ptend  ! dummy command? initialize?
    !call physics_ptend_init (ptend_TOPO) ! Initialize parameterization tendency structure?
    ptend_TOPO%ls = .True.
    ptend_TOPO%lu = .False.
    ptend_TOPO%lv = .False.
    ptend_TOPO%lq = .False.
        ...
        do i=1,ncol
            if (sgh30(i) .lt. 100.0) then
                   sgh_htfrac(i) = 1.0
            elseif (sgh30(i) .lt. 200.0) then
!                   sgh_htfrac(i) = 1.0 - ((sgh30(i) - 100.0) / 100.0)
                   sgh_htfrac(i) = -0.01 * sgh30(i) + 2.0
            else
                   sgh_htfrac(i) = 0.0
            end if
        end do
 ...
 !----------
   ! apply the TOPOTEST forcing here
    do k=1,pver
    do i=1,ncol
        ptend_TOPO%s(i,k) = (HC_deep_ptend%s(i,k) + HC_shallow_ptend%s(i,k)) * (sgh_htfrac(i) - 1.0)
        HCforcing(i,k) = (HC_deep_ptend%s(i,k) + HC_shallow_ptend%s(i,k)) / cpair * (sgh_htfrac(i) - 1.0) ! divided by cpair to obtain the forcing in Kelvin/sec, TOPOTESTX forcing relative to CTR
    end do
    end do
    call physics_update (state, tend, ptend_TOPO, ztodt)
   
    call outfld ('HCforcing',HCforcing, pcols, lchnk)
        ...
        do i=1,ncol
            if (sgh30(i) .lt. 100.0) then
                   sgh_htfrac(i) = 1.0
            elseif (sgh30(i) .lt. 200.0) then
                   sgh_htfrac(i) = -0.008 * sgh30(i) + 1.8
            else
                   sgh_htfrac(i) = 0.2
            end if
        end do
        ...
        do i=1,ncol
            if (sgh30(i) .lt. 100.0) then
                   sgh_htfrac(i) = 1.0
            elseif (sgh30(i) .lt. 200.0) then
                   sgh_htfrac(i) = -0.006 * sgh30(i) + 1.6
            else
                   sgh_htfrac(i) = 0.4
            end if
        end do
        ...
        do i=1,ncol
            if (sgh30(i) .lt. 100.0) then
                   sgh_htfrac(i) = 1.0
            elseif (sgh30(i) .lt. 200.0) then
                   sgh_htfrac(i) = -0.004 * sgh30(i) + 1.4
            else
                   sgh_htfrac(i) = 0.6
            end if
        end do
        ...
        do i=1,ncol
            if (sgh30(i) .lt. 100.0) then
                   sgh_htfrac(i) = 1.0
            elseif (sgh30(i) .lt. 200.0) then
                   sgh_htfrac(i) = -0.002 * sgh30(i) + 1.2
            else
                   sgh_htfrac(i) = 0.8
            end if
        end do
